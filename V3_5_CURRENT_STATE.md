...
> **Status: superseded.** This file is retained as a historical checkpoint
> ledger. Final V3.5 authority is `V3_5_FINAL_CLOSEOUT.md`,
> `V3_5_FINAL_CLOSEOUT.json`, and `V3_5_FINAL_FREEZE_SEAL.json`;
> repository-wide authority is `SHILIU_V0_TO_V3_5_FINAL_CLOSEOUT.md`.

## F1A Preimplementation Recovery Amendment — 2026-07-27

```yaml
authority:
  checkpoint: F1A_PREIMPLEMENTATION_RECOVERY_AMENDMENT
  execution_status: complete
  acceptance_status: pending_v3_5_b_review
  recovery_amendment_status: execution_candidate_ready

incident:
  identity: overbroad_repository_search_entered_frozen_gold_tree
...
filesystem_access:
  policy: F1A_RECOVERY_FILESYSTEM_ACCESS_POLICY.json
  default: deny
...
>
> This Recovery Amendment is the current F1A operational state candidate.
> It invalidates the prior preimplementation attempt without consuming the
> sole Major Cycle, authorizes a fresh Major Cycle 1 in a new Codex Session,
> and freezes the default-deny filesystem policy. It does not start
> implementation, F1B, or Stage 4. Older F1A authorization and next-step
> statements below are historical snapshots.

## P9 Candidate Design Amendment Execution Candidate — 2026-07-27

```yaml
authority:
  checkpoint: P9_Candidate_Design_Amendment_v1
  status: design_amendment_execution_candidate_ready
  acceptance_status: pending_v3_5_b_review
...
> **Current authority notice**
>
> The P9 execute decision and accepted failure mechanisms remain unchanged.
> This Amendment supersedes only the unimplemented fixed 28+4 Candidate
> Design. It freezes `adaptive_bounded_coverage_swap_v1` as the sole active
> implementation candidate, but does not authorize or start F1A. All sections
> below are retained as historical stage snapshots.

## P8 Scoring / Checkpoint 1 Amendment Execution Candidate — 2026-07-27

```yaml
authority:
  checkpoint: Checkpoint_1_Amendment_v1
...
amended_product_initial_baseline:
  retrieval:
    hit_at_1: 0.7142857142857143
    hit_at_3: 0.9285714285714286
    hit_at_5: 0.9285714285714286
    hit_at_10: 1.0
    empty_results: 0
    primary_failures: 0
  ...
  candidate_builder:
    complete_group_coverage:
      numerator: 6
      denominator: 13
      excluded_query_ids: [PQS_V1_Q017]
    required_span_recall: 0.6538461538461539
    required_span_recall_denominator: 13
    required_aspect_coverage: 0.6538461538461539
    required_aspect_coverage_denominator: 13
    primary_failures: 6
...
next:
  formal_step: V3_5_B_REVIEW_P8_SCORING_AND_CHECKPOINT_1_AMENDMENT
  P9_previous_attempt: safely_blocked_due_to_scoring_defect
  P9_status: blocked_pending_v3_5_b_acceptance
  P9_restart_readiness: true
  P9_restarted: false
...
> **Current authority notice**
>
> The Amendment section above is the current operational state candidate
> pending V3.5-B review. It preserves P7 and the P8 Attempt 3 frozen Prediction
> Set, supersedes only the original P8 scoring and Checkpoint 1 derived
> findings, and does not restart P9 or authorize F1A, F1B, or Stage 4. The
> Checkpoint 1, H0-R, and all subsequent sections below are retained as
...
> **Current authority notice**
>
> Only the `H0-R Corrected Handoff State` section above is current
> operational authority for the V3.5-A → V3.5-B handoff.
>
> Every section below this notice is preserved as an immutable historical
> stage snapshot. Historical statements named `Current Stage`, `Next Action`,
> authorization, remaining budget, Held-out, Stage 3A/3B status, or proposed
> repair must not be used to authorize new work.
>
> Current execution authority is defined by the final Handoff Package,
> especially `01_V3_5_B_OPERATING_CONTRACT.md` and
> `03_V3_5_DECISION_AND_ARTIFACT_INDEX.md`, once those files are approved.
