...
> **INVALIDATED — DO NOT EXECUTE.** Post-freeze audit found that the scorer
> compared Gold logical Segment IDs with Runtime physical Segment IDs without
> the required deterministic timeline/ordinal mapping. The apparent generic
> Candidate Builder failure and every metric derived from it are not accepted.
> Stage 3R-F1 is not authorized.

The following payload is retained only as the invalidated output produced before
the scoring-identity defect was detected.
...
```json
{
  ...
  "affected_stage": "candidate_builder",
  "failure_signature": "no_complete_gold_group_in_candidate_set",
  "forbidden_case_specific_logic": true,
  "heldout_access_required": false,
  "proposed_generic_mechanism": "one bounded, query-agnostic candidate coverage expansion within existing budgets",
  "rerun_scope": "all frozen 31 Development cases after separate authorization",
  "v3_retuning_required": false,
  "write_scope": [
    "existing affected stage only",
