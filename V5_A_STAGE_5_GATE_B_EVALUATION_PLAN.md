...
plan_status: draft_pending_main_review
...
该候选替代规划开始时观察到的 `8413958… / 03:06:53`：外部 scheduled sync run 353
在 docs 编写期间完成并改变 DB fingerprint，但 schema、corpus/retrieval counts 和目标
artifact hash 未变。V5-A 未触发或干预该 sync。运行授权必须明确接受上方最新身份；若
届时再次变化仍按下述规则停止，不自动滚动到新 snapshot。

运行获权后必须：

1. 在 stable window 重新核验上述 SHA/count；任一变化即停止，不静默换 snapshot；
2. 把 DB 和用例所需 artifact 复制到独立 eval root，记录复制前后 SHA；
3. 只对 eval DB 执行必要 source schema 10 migration/preflight，绝不初始化 live
   `Application`；
4. eval runtime 的 Task/Trace/Result/Receipt 只写 eval DB；原 corpus/artifact copy 设为
   只读；
5. manifest 固定每个 EvidenceIdentity 的 source version、artifact hash、timeline、
   EvidenceUse 与 validation observation。
...
| `agent_action` | 同上 | disabled | 4,096 tokens | 180s | 选择一个有界研究动作 |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| GB-G-01 | 1 | 8 | 1 / 4 / 2 = 7 | 60k / 18k | 12 min |
| GB-I-01 | 0 | 6 | 1 / 3 / 1 = 5 | 40k / 12k | 10 min |
| GB-H-01（条件） | 1 | 6 | 1 / 3 / 1 = 5 | 40k / 12k | 12 min |

- 必跑总上限：12 logical calls、100k input、30k output、22 分钟；
- 含条件用例总上限：17 logical calls、140k input、42k output、34 分钟；
- transport retry：每 logical call 最多 1 次；HTTP attempt 上限分别为 24 / 34；
- 价格必须在授权时固定到 manifest；名义费用估算 `<= US$2.00`，预留费用达到
  `US$3.50` 时不得开始下一调用，绝对硬上限 `US$4.00`；无法在调用前可靠计价则整个
  eval 标为 `evaluation_invalid_run` 并不运行；
...
4. 是否接受 12（或含 HITL 17）logical calls、US$4.00、34 分钟硬上限；
...
```yaml
gate_A_status: accepted_by_v5_main
gate_B_plan_status: draft_pending_main_review
