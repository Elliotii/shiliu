...
class _CompatibilityClaimVerifier:
    """Network-free semantic observation for legacy behavior-focused tests."""
    ...
    def verify(self, claims):
        return ClaimVerifierBatch(
            ...
            verifier_reference="v4-fast-deterministic-test-fake",
        )


def _application(app_paths, provider: _Provider, *, claim_verifier=...):
    ...
    core._ask_service = AskService(
        ...
        claim_verifier=(
            _CompatibilityClaimVerifier()
            if claim_verifier is ...
            else claim_verifier
        ),
    )
