...
AnswerExecutionOutcome = Literal[
    ...
    "claim_verifier_failed",
    "claim_verifier_unknown",
]
...
class AnswerTrustDisposition(_StrictModel):
    claim_id: str
    answer_block_id: str
    outcome: Literal["allow", "downgrade", "remove"]
    ...
    support_class: Literal[
        "exact_current_evidence_excerpt_v1",
        "semantic_observation",
        "unobserved",
    ]


class AnswerTrustSummary(_StrictModel):
    schema_version: Literal["v5.6-answer-trust-v1"] = "v5.6-answer-trust-v1"
    claim_unit: Literal["answer_block"] = "answer_block"
    claim_support_schema_version: Literal["v5.6-bounded-claim-support-v1"] = (
        "v5.6-bounded-claim-support-v1"
    )
    overall_outcome: Literal["allow", "partial", "insufficient"]
    deterministic_passes: Literal[1] = 1
    semantic_passes: int = Field(default=0, ge=0, le=1)
    verifier_logical_calls: int = Field(default=0, ge=0, le=1)
    verifier_http_attempts: int = Field(default=0, ge=0, le=2)
    semantic_answer_repairs: Literal[0] = 0
    verifier_state: Literal[
        "not_needed", "disabled", "committed", "failed", "unknown"
    ]
    input_hash: str
    answer_snapshot_hash: str
    answer_version: int = Field(default=1, ge=1)
    previous_answer_snapshot_hash: str | None = None
    dispositions: list[AnswerTrustDisposition] = Field(default_factory=list)
    reason_codes: list[str] = Field(default_factory=list)
...
class AskResponse(_StrictModel):
    ...
    trust_summary: AnswerTrustSummary | None = None
    answer_version: int = Field(default=1, ge=1)
