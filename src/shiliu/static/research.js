(() => {
  ...
  const renderTask = product => {
    ...
    root.querySelector('[data-research-save-draft]').disabled = product.task.status !== 'terminal';
  ...
  root.querySelector('[data-research-save-draft]').addEventListener('click', async event => {
    if (!activeTaskId) return;
    const button = event.currentTarget;
    const status = root.querySelector('[data-research-draft-status]');
    ...
    try {
      const data = await requestJson('/api/knowledge/drafts', {
        method: 'POST',
        body: JSON.stringify({
          source_kind: 'research_task', source_id: activeTaskId,
          command_id: commandId('research-draft'),
        }),
      });
      ...
      status.textContent = error.message;
      button.disabled = current?.task?.status !== 'terminal';
    }
  });
