...
document.querySelectorAll('[data-source-history]').forEach(button=>button.addEventListener('click',async()=>{button.disabled=true;try{const data=await postJson(`/api/sources/${button.dataset.sourceHistory}/history-drain`);sourceStatus.textContent='历史导入已进入后台。';pollRun(data.run_id,sourceStatus,button)}catch(error){sourceStatus.textContent=error.message;button.disabled=false}}));
document.querySelectorAll('[data-update-source-coverage]').forEach(button=>button.addEventListener('click',async()=>{button.disabled=true;const sourceId=button.dataset.updateSourceCoverage,selected=document.querySelector(`[data-source-coverage="${sourceId}"]`).value;let history_policy=selected,history_limit=null;if(selected.startsWith('latest_n')){history_policy='latest_n';history_limit=Number(selected.split(':')[1])}try{const data=await postJson(`/api/sources/${sourceId}/history-coverage`,{history_policy,history_limit});sourceStatus.textContent=`历史范围已更新，待处理 ${data.history_pending} 条。`;setTimeout(()=>location.reload(),500)}catch(error){sourceStatus.textContent=error.message;button.disabled=false}}));
