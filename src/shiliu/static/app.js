...
modelButton?.addEventListener('click',async()=>{const fields=new FormData(setupForm);modelButton.disabled=true;try{const model=fields.get('formal_summary_model')||fields.get('model');const data=await postJson('/api/setup/models',{base_url:fields.get('base_url'),api_key:fields.get('api_key'),model,thinking_enabled:true,reasoning_effort:'high'});const select=document.querySelector('[data-model-list]');select.innerHTML='<option value="">选择模型</option>';data.models.forEach(value=>select.add(new Option(value,value)));select.onchange=()=>{if(select.value&&!setupForm.elements.fast_transcript_model.value)setupForm.elements.fast_transcript_model.value=select.value}}catch(error){alert(error.message)}finally{modelButton.disabled=false}});

document.querySelectorAll('[data-test-role]').forEach(button=>button.addEventListener('click',async()=>{const fields=new FormData(setupForm),status=document.querySelector('[data-model-status]'),role=button.dataset.testRole,isFast=role==='fast_transcript_model';button.disabled=true;status.textContent=`正在测试 ${fields.get(role)}…`;try{const data=await postJson('/api/setup/test-provider',{base_url:fields.get('base_url'),api_key:fields.get('api_key'),model:fields.get(role),thinking_enabled:!isFast,reasoning_effort:'high'});status.textContent=`${role} 连接成功：${data.result}`}catch(error){status.textContent=`${role} 连接失败：${error.message}`}finally{button.disabled=false}}));

const setupPayload=fields=>({content_dir:fields.get('content_dir'),favorite_id:fields.get('favorite_id')?Number(fields.get('favorite_id')):null,favorite_title:setupForm.elements.favorite_id?.selectedOptions[0]?.dataset.title||'',base_url:fields.get('base_url'),api_key:fields.get('api_key'),model:fields.get('formal_summary_model'),fast_transcript_model:fields.get('fast_transcript_model'),formal_transcript_model:fields.get('fast_transcript_model'),formal_summary_model:fields.get('formal_summary_model'),thinking_enabled:true,reasoning_effort:'high'});
