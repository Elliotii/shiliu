...
DOMAIN_DRAFT_A_MANIFEST_VERSION = "domain-draft-a-manifest-v1"
...
def evaluate_domain_draft_a(draft: DomainDraftA) -> dict[str, Any]:
    nodes = list(draft.nodes)
    by_id = {node.draft_node_id: node for node in nodes}
    children: dict[str | None, list[DomainDraftANode]] = defaultdict(list)
    for node in nodes:
    ...
    source_cluster_counts = {
        node.draft_node_id: len(set(node.source_cluster_ids)) for node in nodes
    }
    unique_source_clusters = {
        cluster_id for node in nodes for cluster_id in node.source_cluster_ids
    }
...
def _draft_tree_markdown(draft: DomainDraftA) -> str:
    children: dict[str | None, list[DomainDraftANode]] = defaultdict(list)
    for node in draft.nodes:
        children[node.parent_id].append(node)
    ...
    for root in children[None]:
        lines.append(
            ...
            f"[sources: {', '.join(root.source_cluster_ids)}; evidence: {len(root.evidence_pool_ids)}]"
        )
        for child in children[root.draft_node_id]:
            lines.append(
                f"  - **{child.name}** (`{child.status}`) — {child.definition} "
                f"[sources: {', '.join(child.source_cluster_ids)}; evidence: {len(child.evidence_pool_ids)}]"
            )
...
def freeze_domain_draft_a(run_dir) -> dict[str, Any]:
    draft = DomainDraftA.model_validate_json(
    ...
    review = json.loads(
        (run_dir / "domain-draft-a-hierarchy-review.json").read_text(encoding="utf-8")
    )
    concurrency = json.loads(
        (run_dir / "domain-draft-a-concurrency-audit.json").read_text(encoding="utf-8")
    )
    validate_domain_draft_a(draft, final_clusters=clusters)
    evaluation = evaluate_domain_draft_a(draft)
    if not evaluation["passed"] or not review.get("freeze_eligible"):
        raise PipelineError(
            "Draft A 尚未通过结构与 Reviewer Freeze Gate",
    ...
    _write_json_once(run_dir / "domain-draft-a.json", payload)
    tree_path = run_dir / "domain-draft-a-tree.md"
    tree_text = _draft_tree_markdown(draft)
    if tree_path.exists():
        if tree_path.read_text(encoding="utf-8") != tree_text:
            raise FileExistsError(tree_path)
    else:
    ...
    _write_json_once(run_dir / "domain-draft-a-hierarchy-revision.json", revision_record)
    manifest = {
        "version": DOMAIN_DRAFT_A_MANIFEST_VERSION,
        "frozen_at": _utc_now(),
        "reviewer_verdict": review["verdict"],
        "blocking_count": review["blocking_count"],
        "dimension_leakage_count": int(review.get("dimension_leakage_count") or 0),
        ...
        "stable_node_count": 0,
        "excluded_cluster_count": len(draft.excluded_cluster_ids),
        "remaining_concerns": review.get("non_blocking_concerns") or [],
        "provider_usage_known_minimum": concurrency["known_minimum_usage"],
        "provider_usage_unknown_response_count": concurrency["unknown_usage_response_count"],
        "artifact_hashes": {
            ...
            "hierarchy_review": _stable_hash(review),
            "tree": _stable_hash(tree_text),
        },
        "source_hashes": {
            "m3_final_clusters": _stable_hash(clusters),
            "provider_draft": _stable_hash(payload),
            "concurrency_audit": _stable_hash(concurrency),
        },
        "next_stage": "trial_assignment",
        "trial_assignment_started": False,
    }
    _write_json_once(run_dir / "domain-draft-a-manifest.json", manifest)
    return manifest
