...
class ResearchProductService:
    ...
    def intake_draft_materialization_evidence(
        ...
        *,
        ...
        """Start and seed the Task's initial Attempt from server-owned Draft data."""

        lock = self._run_lock(task_id)
        if not lock.acquire(blocking=False):
            raise ResearchConflict("Task is already being advanced by the local runner")
        try:
            raw = self.kernel.get_task(task_id)
            task = raw["task"]
            if str(task["status"]) not in {"ready", "running", "terminal"}:
                raise ResearchUnsafeState("Draft materialization Task is not intakeable")
            if not task.get("owner_id"):
                if str(task["status"]) == "terminal":
                    raise ResearchUnsafeState("terminal Task has no replayable Draft intake owner")
                ...
                raise ResearchConflict("Draft materialization Task has a different owner")

            attempts = list(raw["attempts"])
            if not attempts:
                if str(task["status"]) == "terminal":
                    raise ResearchUnsafeState("terminal Task has no Draft intake Attempt")
                self.kernel.start_attempt(
                    task_id=task_id,
                    command_id=_derived_command(command_id, "draft-intake-start"),
                    owner_id=self.runner_id,
                ...
                attempts = list(raw["attempts"])
            if len(attempts) != 1:
                raise ResearchConflict("Draft materialization intake is bound to one initial Attempt")
            attempt = attempts[0]
            if (
                str(attempt["cause"]) != AttemptCause.INITIAL.value
                or int(attempt["ordinal"]) != 1
            ):
                raise ResearchConflict("Draft materialization intake Attempt is not initial")
            return self.inner.intake_draft_materialization_evidence(
                task_id,
                ...
                owner_epoch=int(task["owner_epoch"]),
                expected_state_version=int(task["state_version"]),
                draft_binding=dict(draft_binding),
                spans=tuple(spans),
            )
        finally:
            lock.release()
    ...
    def create_provider_task(
        ...
        *,
        ...
        try:
            ...
            for key, expected in continuation_policy.items():
    ...
    def _continuation_policy(
        ...
        *,
        ...
        continuation_envelope: ContinuationEnvelope | None,
    ) -> dict[str, Any]:
        ...
        if evidence_decision is None or continuation_envelope is None:
            return {}
        try:
            validate_continuation_consumption(
                evidence_decision,
                ...
                current_scope=continuation_envelope.normalized_scope,
                target=ContinuationTarget.RESEARCH,
            )
        except ContinuationContractError as exc:
            raise ResearchValidationError(
                f"Research continuation rejected ({exc.code}): {exc}"
            ) from exc
        return {
            "evidence_decision": decision_to_persistence_projection(evidence_decision),
            "continuation_envelope": continuation_envelope.model_dump(mode="json"),
            "adaptive_directive": decision_directive(evidence_decision),
